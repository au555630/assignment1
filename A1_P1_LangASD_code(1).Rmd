...
taken_train= read.csv("C:/Users/Viki/Documents/egyetem/exp.met/port1/assignment1/token_train.csv")
```
...
demo_train = plyr::rename(demo_train, c("Child.ID" = "SUBJ"))
demo_train = plyr::rename(demo_train, c("Visit" = "visit"))
LU_train = plyr::rename(LU_train, c("VISIT" = "visit"))
taken_train = plyr::rename(taken_train, c("VISIT" = "visit"))
...
taken_train$visit=str_extract(taken_train$visit, "\\d")
...
demo_train$SUBJ <- gsub("\\.", "", demo_train$SUBJ, perl = TRUE)
LU_train$SUBJ <- gsub("\\.", "", LU_train$SUBJ, perl = TRUE)
taken_train$SUBJ <- gsub("\\.", "", taken_train$SUBJ, perl = TRUE)
...
```{r}
demo_train = plyr::rename(demo_train, c("SUBJ" = "id"))
LU_train = plyr::rename(LU_train, c("SUBJ" = "id"))
taken_train = plyr::rename(taken_train, c("SUBJ" = "id"))
...
taken_train1= select(taken_train, id, visit, types_MOT, types_CHI, tokens_MOT, tokens_CHI)
```
...
```{r}
#1.
#my version (after Celine told us that we did the wrong thing)
one = na.omit(final) #get out na-s
uno_means= summarise(group_by(one, id), save_mean=mean(CHI_MLU)) # get the mean of CHI_MLU belonging to the same id
...
not_all= three %>%
    ...
    tally() %>% #counts rows
    filter(n < 6)
#shortcut for the pipe thing (control+shift+m)
