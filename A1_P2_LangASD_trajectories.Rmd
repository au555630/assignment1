...
#age
age= glm(Age ~ Diagnosis, family="gaussian", visit_1) #use gaussian if continous
summary(age)

#intercept: 32.9990 diagnosisTD: -12.6296
#the intercept is the mean of one variable (the first in the alphabet), the next line shows the difference between the means.
#significant difference - not a problem, because they are at the "same" level when the study starts

#ethnicity
ethnicity= glm(Ethnicity ~ Diagnosis, family="quasibinomial", visit_1) #too many things. what and how?
summary(ethnicity)
#no significant difference

#gender
gender= glm(Gender ~ Diagnosis, family="binomial", visit_1) #use binomial if only two possib.
summary(gender)
#no significant difference
...
#plots
ggplot(visit_1, aes(Diagnosis, Age)) + geom_violin()

ggplot(visit_1, aes(Diagnosis, verbal_IQ)) + geom_violin()

ggplot(visit_1, aes(Diagnosis, nonverbal_IQ)) + geom_violin()

ggplot(visit_1, aes(Diagnosis, CHI_MLU)) + geom_violin()
ggplot(visit_6, aes(Diagnosis, CHI_MLU)) + geom_violin()
...
- Which variable(s) should be included as fixed factors? visit, diagnosis (gender, ethnicity)
