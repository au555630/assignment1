...
Therefore, out of these three models, the quadratic one is the best. Hypothesis 2 is false, mothers do speak differently to their children is they are autistic. 
...
#do we need the random effect?
#model12= lm(CHI_MLU ~ ADOS + visit + MOT_MLU + types_CHI + types_MOT + verbal_IQ , port1_data)
#summary(model12)
...
#get out all the variables that explain it too obviously (types, verbal IQ)
...
I decided to put autism severity as the first and most important predictor. I used this instead of the diagnosis, because I believe that autism severity as a scale (how autistic the child is?) is a better description than deciding whether somebody is autistic or not. And I supposed that being autistic or not would have the biggest effect on language development. The next variable is visit, in other words the progress of the time, which I think should be just as important, as being autistic or not. Next I used the mean length of utterance used by the mothers, because they are the biggest influence the children receive when it comes to speaking. If they use shorter words, the children has little possibility to learn long ones in my opinion. The amount of unique words used by both children and mothers should have a big effect on how long the words they use are. The verbal IQ is also important. The importance of being autistic or not (diagnosis) should have an effect on how children develop differently over time, therefore I included the interaction of diagnosis and visit. I included the random slope because the children start with different vocabularies and they develop differently (some are faster, others are slower).
Visit is not a significant predictor (ß= 0.01, SE= 0.001, p =0.499), MLU of the mothers is a  highly significant predictor, (ß= 0.034, SE= 0.004, p < .001), unique words for children is a highly significant predictor, (ß= 0.0007, SE= 0.00005, p < .001), unique words for mothers is a highly significant predictor, (ß= -0.001, SE= 0.003, p < .001), verbal IQ is a highly significant predictor, (ß= 0.02, SE= 0.005, p < .001), the interaction of diagnosis and visit is a highly significant predictor also, (ß= 0.1, SE= 0.026, p < .001). The model explains 84% of the data using the fixed effects only (R^2=0.84), and 88% using both fixed and random effects (R^2= 0.88). The little difference between them shows, that the model is good, at least much better, than the previous ones.