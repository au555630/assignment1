...
rmse(model8, port1_data)
rmse(model9, port1_data)
```
...
```{r}
rmse(model8, test_data)
rmse(model9, test_data)
...
croval= data.frame(row.names = 1:4)

for (i in folds) {
  three_fold = subset(port1_data, id %not in% i)
  one_fold= subset(port1_data, id %in% i)
  three_fold %+% model10
  saving =rmse(model10, one_fold)
  croval= cbind(croval, saving) #not working - cbind either. there should be a way to attach them next to each other??
  #the column names doesn't make sense
  #it keeps saving the same numbers?! shouldn't it be different?
  #wrong with cbind and rbind also, keeps saving the same numbers
