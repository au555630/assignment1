...
#old-final model (before discussion in class, kept in out of curiousity)
...
#both are bad, but new-final is worse
...
The first model I tested was the old-final model, to see how a "bad model" performs on the test. Its performance on the train data is not so bad, the root mean square error is 0.287. Its performace on the test data is also not good, the root mean square error is 0.461.
The second model was the new-final model, the best model I could create. Its performance on the train data is bad, the root mean square error is 0.324. Its performace on the test data is really bad, it does not fit the data, the root mean square error is 0.755.
...
#model8, the really bad old-final model
#model8= lmer(CHI_MLU ~ ADOS + visit + MOT_MLU + types_CHI + types_MOT + verbal_IQ + Diagnosis*visit + (1 + visit|id), port1_data)
...
for (i in folds) {
  three_fold = subset(port1_data, id %not in% i)
  one_fold= subset(port1_data, id %in% i)
  model8= lmer(CHI_MLU ~ ADOS + visit + MOT_MLU + types_CHI + types_MOT + verbal_IQ + Diagnosis*visit + (1 + visit|id), three_fold)
...
for (i in folds) {
  three_fold = subset(port1_data, id %not in% i)
  one_fold= subset(port1_data, id %in% i)
  model9= lmer(CHI_MLU ~ ADOS + visit + Diagnosis*visit + nonverbal_IQ + I(visit^2) + (1 + visit|id), three_fold)
...
The old-final model is the one that performed the best on the cross-validation. The root mean square error for the train data is 0.29, and for the test data is 0.43.
The new-final model is the second best model. The root mean square error for the train data is 0.32, and for the test data is 0.64.
Even though the old-final model is better than the new-final model, the old one is wrong in using variables that are too good predictors, like the unique words the child says. Therefore the new-final model is still the best model I could make.
I could not think of any other model that gave me better cross-validated results.
...
bernie$prediction=predict(model10, bernie)

modelr::rmse(model10, bernie)
...
bernie$better_prediction=predict(model9, bernie)

modelr::rmse(model9, bernie)
...
Unfortunately the basic model is a model that is better at predicting Bernie's development than my best model. 
